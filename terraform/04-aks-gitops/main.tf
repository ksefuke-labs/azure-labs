...
resource "azurerm_kubernetes_flux_configuration" "main" {
  ...
  git_repository {
    url             = "ssh://git@github.com/ksefuke-labs/azure-labs"
...
resource "azurerm_key_vault" "apollo_vault" {
  name                = "kv-apollo-staging"
  location            = azurerm_resource_group.aks.location
  resource_group_name = azurerm_resource_group.aks.name
...
resource "azurerm_role_assignment" "kv_admin" {
  scope                = azurerm_key_vault.apollo_vault.id
...
resource "azurerm_key_vault_secret" "customer1_db_user" {
  ...
  value        = "app"
  key_vault_id = azurerm_key_vault.apollo_vault.id
...
output "key_vault_name" {
  value = azurerm_key_vault.apollo_vault.name
}

output "key_vault_uri" {
  value = azurerm_key_vault.apollo_vault.vault_uri
}
